...
public class Request {
    ...
    public void editProfile(int userId, String email, String name, String password, String phoneNumber, String token, RequestListener<RespUser> listener) {
        HashMap<String, String> map = new HashMap<>();
        map.put("email", email);
        ...
        map.put("phoneNumber", phoneNumber);
        map.put("token", token);
        Call<RespUser> callbackCall = api.editProfile(map);
        callbackCall.enqueue(new Callback<RespUser>() {
            @Override
            public void onResponse(@NonNull Call<RespUser> call, @NonNull Response<RespUser> response) {
                RespUser resp = response.body();
