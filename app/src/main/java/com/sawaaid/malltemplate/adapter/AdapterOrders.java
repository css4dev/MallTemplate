...
public class AdapterOrders extends RecyclerView.Adapter<RecyclerView.ViewHolder> {
    ...
    private AdapterListener<Order> listener;
    ...
    public void setListener(AdapterListener<Order> listener) {
        this.listener = listener;
    }


    public static class OriginalViewHolder extends RecyclerView.ViewHolder {
        AutofitTextView orderNumberTextView, orderDateTextView,
                orderTotalPriceTextView, orderStatusTextView;
        LinearLayout orderLinearLayout;

        public OriginalViewHolder(View v) {
            super(v);
            orderNumberTextView = v.findViewById(R.id.orderNumberTextView);
            orderDateTextView = v.findViewById(R.id.orderDateTextView);
            orderTotalPriceTextView = v.findViewById(R.id.orderTotalPriceTextView);
            orderStatusTextView = v.findViewById(R.id.orderStatusTextView);
            orderLinearLayout = v.findViewById(R.id.orderLinearLayout);
    ...
    public AdapterOrders(Context context, RecyclerView view, int page) {
    ...
    public RecyclerView.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ...
        if (viewType == VIEW_ITEM) {
            ...
            vh = new AdapterOrders.OriginalViewHolder(v);
        } else {
            View v = LayoutInflater.from(parent.getContext()).inflate(R.layout.item_loading, parent, false);
            vh = new AdapterOrders.ProgressViewHolder(v);
        }
        return vh;
    }
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, @SuppressLint("RecyclerView") int pos) {
        ...
        if (holder instanceof AdapterOrders.OriginalViewHolder) {
            AdapterOrders.OriginalViewHolder vItem = (AdapterOrders.OriginalViewHolder) holder;
            Order p = items.get(position);

            vItem.orderNumberTextView.setText(p.id + "");
            vItem.orderDateTextView.setText(p.orderDate);
            vItem.orderTotalPriceTextView.setText(p.totalPrice + "");
            vItem.orderStatusTextView.setText(p.status + "");

            vItem.orderLinearLayout.setOnClickListener(view -> {
                if (listener == null) return;
                listener.onClick(view, null, p, position);
            });

        } else {
            final AdapterOrders.ProgressViewHolder v = (AdapterOrders.ProgressViewHolder) holder;
